...
# **Omega** 
## 背景
在大规模的集群中，为了充分利用硬件资源，通常会将各种类型(CPU密集，IO密集，批量处理，低延迟 等等)的workloads运行在同一台机器上，同时随着集群规模的增大，需要调度的任务的规模也线性增大。当下存在的中央式调度器（Monolithic scheduler）和双层调度器（Two-level scheduler）已经很难满足规模迅速增长和需求变更快速响应的需要。由此带来的后果是限制了新特性的部署、降低了效率和可用性，最终会限制集群的发展。而大规模的计算集群是很昂贵的，所以充分利用它们很有必要。所以在这种情况下，Google公司提出了他们的第三代集群管理系统——Omega。
...
## 评价
Omega作为Google的第三代集群管理系统，针对解决的问题的是在大集群中资源利用率的提高和可扩展性的提高。而对这些问题，Omega也展示了很好的处理效果，表现出与前两代系统不同的优势。但是，Omega在许多方面仍然还不够成熟，由于没有中央调度进程的存在，不太可能实现绝对的公平调度，避免饥饿和减少冲突等方面需要更细化的方案和策略。而且Omega与Mesos/YARN架构的不同主要体现在资源分配模块，因此，我们可以通过改造Mesos或者YARN的“Resource Master”模块将其改造成一个类似于Omega的系统。
...
# Apollo
## 简介
有效地调度数据并行计算在云计算的集群中对性能、系统吞吐量和资源利用率要求很高。随着集群大小增长和更复杂的工作负载，它变得更加具有挑战性。Apollo这个可扩展和协调的调度框架已经部署在微软的生产集群上，每天有数以百万计的计算和成千上万的任务有效地被完成。
## 特征
### 规模调度
* 阿波罗作为微软的分布式计算平台的底层调度框架，支持各种数据的大规模数据分析需要。一个典型的集群包含数万服务器和网络。分布式文件系统把数据存储在可分布和复制的部分中。所有计算任务都是用SCOPE，一个类似SQL的高级脚本语言，用用户定义的处理逻辑进行扩充。优化器将任务转换为物理执行，表示为有向无环图（DAG）的方案任务，每个图代表一个基本的计算单元，数据作为任务的边缘流动。任务在不同的分区上执行相同的计算。

### 架构
* 为了支持工作负载所需的规模和调度速率，Apollo采用了分布式和可协调的体系结构，其中每个作业的调度是独立执行的，并且合并了聚合的全局集群负载信息。
* 作业管理器（JM），也称为调度器，被分配来管理每个作业的生命周期。每个JM使用的全局集群负载信息通过Apollo框架中两个附加实体的合作来提供：每个集群的资源监视器（RM）和每个服务器上的进程节点（PN）。在每个服务器上运行的PN进程负责管理该服务器上的本地资源并执行本地调度，而RM连续地聚合来自跨集群的PN的负载信息，为每个JM提供集群状态的全局视图，以便进行知情的调度决定。

### 工程经验
* 我们的经验表明，集群负载不大可能在两次连续运行之间急剧变化。我们还修改了基线调度程序，以便使用Apollo的逻辑对任务运行时间和资源需求做出准确的估计，以便Apollo能够在这种混合模式环境中充分地执行任务。这使得我们能够在设计和实验阶段从早期暴露到大规模环境中获得性能数据。
## 优缺点
### 优点
#### 分散式设计
* 阿波罗的完全分散式设计允许每个调度器与其他调度器或阿波罗本身的其他版本并肩运行。这种工程敏捷性是至关重要的，并且允许我们在相同的生产环境中以规模比较不同调度器的性能。我们对生产作业进行采样并重新运行两次，一次使用基线调度器，另一次使用Apollo调度器，以比较作业性能。然而，该方法导致人工资源争用，因为两个作业读取相同的输入。相反，我们选择一个接一个地运行这两个工作。
#### 修正机制
* 在阿波罗中，每个JM可以独立地在高频率上调度任务，在过程中没有延迟。这对于在工作量中调度大量的小任务至关重要。然而，由于调度的分布式性质，几个JMS可以同时做出竞争性决策。此外，用于调度决策的信息（如等待时间矩阵）可能过时；任务等待时间和运行时间可能低于或高估。阿波罗有内置的机制来解决这些挑战，并动态调整调度决策与新的信息。
#### 基于估计的调度
* JM必须决定使用RM提供的聚合视图中的等待时间矩阵以及要调度的任务的个别特征来调度特定任务的服务器。Apollo必须考虑影响调度决策质量的各种（经常是冲突的）因素，并且使用基于估计的方法在单个统一模型中这样做 
#### 机会调度
* 如果利用了所有令牌并且仍有待调度的任务，则可以应用机会调度来调度机会任务。通过在每个服务器上以较低的优先级运行机会任务，可以防止常规任务的性能下降，并且如果服务器处于资源压力之下，则可以抢占或终止任何机会任务。
